...
export const classifyHoldOperation = (row: any): HoldOperation | null => {
  const explicitCode = normalizeText(readFirst(row, ["Nghiệp vụ", "NGHIỆP VỤ", "Nghiep vu", "nghiepVu", "Loại"]));
  ...
  const status = normalizeText(`${row?.["Trạng thái"] || ""} ${row?.["Tình trạng thanh toán"] || ""}`);
