...
CREATE TABLE IF NOT EXISTS nhan_vien (
  ...
  phong_ban TEXT,
  ngay_vao_lam DATE,
  trang_thai TEXT DEFAULT 'Dang lam viec',
  created_at TIMESTAMPTZ DEFAULT timezone('utc', now()),
  bank_number_acc TEXT
);
...
BEGIN 
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='nhan_vien' AND column_name='ma_nv') THEN
    ALTER TABLE nhan_vien ADD COLUMN ma_nv TEXT;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='nhan_vien' AND column_name='ho_ten') THEN
    ALTER TABLE nhan_vien ADD COLUMN ho_ten TEXT;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='nhan_vien' AND column_name='bank_number_acc') THEN
    ALTER TABLE nhan_vien ADD COLUMN bank_number_acc TEXT;
  END IF;
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='nhan_vien' AND column_name='employee_id') THEN
    EXECUTE 'UPDATE nhan_vien SET ma_nv = COALESCE(NULLIF(ma_nv, ''''), employee_id) WHERE NULLIF(employee_id, '''') IS NOT NULL';
  END IF;
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='nhan_vien' AND column_name='full_name') THEN
    EXECUTE 'UPDATE nhan_vien SET ho_ten = COALESCE(NULLIF(ho_ten, ''''), full_name) WHERE NULLIF(full_name, '''') IS NOT NULL';
  END IF;
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='nhan_vien' AND column_name='bank_account_number') THEN
    EXECUTE 'UPDATE nhan_vien SET bank_number_acc = COALESCE(NULLIF(bank_number_acc, ''''), bank_account_number) WHERE NULLIF(bank_account_number, '''') IS NOT NULL';
  END IF;
END $$;
...
export async function clearSupabaseData() {
  ...
  const { error: error2 } = await supabase.from("nhan_vien").delete().not("id", "is", null);
...
export async function syncEmployeesToSupabase(
  ...
  const mappedRows = employeeData.map((e) => {
    ...
    return {
      ma_nv: employeeId,
      ho_ten: fullName,
      bank_number_acc: bankAccountNumber,
    };
  }).filter((e) => e.ma_nv);
