...
const MktLocalNorthPivotTableComponent: React.FC<MktLocalNorthPivotTableProps> = ({
  ...
  const { editingCell, editValue, beginEdit, setEditValue, commitEdit, cancelEdit } =
    useTableCellEditor<MktLocalNorthPivotTableRow, { rowKey: string; field: string }>(onCellChange);
  const [hiddenColumns, setHiddenColumns] = useState<Set<string>>(() => {
    try { return new Set(JSON.parse(window.localStorage.getItem("dt_hidden_timesheet_pivot") || "[]")); }
    catch { return new Set(); }
  });
  useEffect(() => {
    window.localStorage.setItem("dt_hidden_timesheet_pivot", JSON.stringify([...hiddenColumns]));
  }, [hiddenColumns]);
  const columns = useMemo(() => [
    { key: "__no", label: "No." }, { key: "business", label: "Business" },
    { key: "chargeToCenterMkt", label: "L07" },
    ...types.map((type) => ({ key: type, label: type })), { key: "__total", label: "Grand Total" },
  ], [types]);
  const visibleTypes = types.filter((type) => !hiddenColumns.has(type));
  const identityColumnCount = ["__no", "business", "chargeToCenterMkt"].filter((key) => !hiddenColumns.has(key)).length;
  ...
  const startEditing = (
    ...
    beginEdit({ rowKey: rowKeyOf(row), field }, row, field, value);
  };
  ...
  const totalTableWidth = (hiddenColumns.has("__no") ? 0 : noColumnWidth) + (hiddenColumns.has("business") ? 0 : 140)
    + (hiddenColumns.has("chargeToCenterMkt") ? 0 : 180) + visibleTypes.length * 120 + (hiddenColumns.has("__total") ? 0 : 150);
    ...
    <div className="flex-1 flex flex-col min-h-0 bg-transparent overflow-hidden">
          ...
          <div className="app-table-title-line">
            {onToggleSidebar ? (
              <button
                ...
                type="button"
              >
                <TableInitialMark label="PIVOT TIMESHEET MKT LOCAL NORTH COST" />
              </button>
            ) : (
      ...
      <div className="pivot-table-container flex min-h-0 flex-1 flex-col overflow-hidden bg-[var(--table-data-bg,var(--card,#fff))]">
        <div className="table-body-region relative min-h-0 flex-1 overflow-auto custom-scrollbar">
          ...
          >
          <colgroup>
            {!hiddenColumns.has("__no") && <col style={{ width: noColumnWidth }} />}
            {!hiddenColumns.has("business") && <col style={{ width: 140 }} />}
            {!hiddenColumns.has("chargeToCenterMkt") && <col style={{ width: 180 }} />}
          ...
          <tbody className="divide-y-0">
            {paginatedRows.length === 0 ? (
              <tr>
                <td colSpan={Math.max(1, columns.length - columns.filter((column) => hiddenColumns.has(column.key)).length)} className="py-8 text-center text-muted-foreground text-xs bg-card">
                  ...
                  {visibleTypes.map((type) => (
                    ...
                    >
                      {isEditing(row, type) ? (
                        ...
                        formatMoneyVND(row.values[type] ?? 0).replace(" ₫", "")
                      )}
                    </td>
                  ))}
      ...
      >
        <div className="flex min-w-0 items-center gap-3">
          ...
          <TableColumnVisibility columns={columns} hidden={hiddenColumns} onChange={setHiddenColumns} />

          <span className="truncate text-[12px] font-medium text-muted-foreground border-l border-border pl-3">
